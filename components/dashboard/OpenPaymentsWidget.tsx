...
import { Wallet, User, Calendar, ChevronRight } from "lucide-react";
...
interface OpenPaymentsWidgetProps {
  payments: Payment[];
  totalCount: number;
}
...
export default function OpenPaymentsWidget({
  payments,
  totalCount,
}: OpenPaymentsWidgetProps) {
    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm p-6 hover:shadow-md transition-shadow duration-200">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500">
              {totalCount > 0 ? `${totalCount} heute ausstehend` : "Heute"}
            </p>
          </div>
        </div>
        {totalCount > 0 && (
      ...
      {payments.length === 0 ? (
        <div className="text-center py-8">
          ...
          <p className="text-slate-500 text-sm">Keine offenen Zahlungen heute</p>
          <p className="text-slate-400 text-xs mt-1">Alle heutigen Zahlungen sind beglichen</p>
        </div>
      ) : (
        <>
          <div className="space-y-2 max-h-[220px] overflow-y-auto">
            {payments.map((payment) => (
              <div
                ...
                className="flex items-center justify-between p-3 rounded-lg hover:bg-slate-50 transition-colors duration-200 cursor-pointer"
              >
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-3 mt-1.5 ml-8 text-xs text-slate-500">
                    ...
                    <span className="flex items-center gap-1">
                      ...
                      {format(new Date(payment.startTime), "d. MMM", {
                        locale: de,
                      })}
                    </span>
                  </div>
                ...
                <div className="flex items-center gap-1 flex-shrink-0 ml-2">
                  <div className="w-5 h-5 rounded-full bg-slate-200 flex items-center justify-center text-[10px] font-medium text-slate-600">
                    {payment.instructor.firstName[0]}
                  </div>
                </div>
              </div>
            ))}
